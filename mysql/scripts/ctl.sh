...
MYSQL_PIDFILE=/opt/moodle-3.10.0-0/mysql/data/mysqld.pid

MYSQL_START="/opt/moodle-3.10.0-0/mysql/bin/mysqld_safe --defaults-file=/opt/moodle-3.10.0-0/mysql/my.cnf --mysqld=mysqld.bin --socket=/opt/moodle-3.10.0-0/mysql/tmp/mysql.sock  --datadir=/opt/moodle-3.10.0-0/mysql/data --log-error=/opt/moodle-3.10.0-0/mysql/data/mysqld.log  --pid-file=$MYSQL_PIDFILE --lower-case-table-names=1 "
...
start_mysql() {
    ...
    if [ $ERROR -eq 0 ]; then
	echo "$0 $ARG: mysql  started at port 3307"
	sleep 2
    else
